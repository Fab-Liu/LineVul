...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):
        step = torch.tensor(self.examples[i].input_ids)
        # print(self.examples[i].label)
        batch = torch.tensor(self.examples[i].label)
        return step, batch
...
def train2(args, train_dataset, model, tokenizer, eval_dataset):
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1'] > best_f1:
                        ...
                        model_to_save = model.module if hasattr(
                            model, 'module') else model
                        output_dir = os.path.join(
                            output_dir, '{}'.format(str(args.model_name) + '_new'))
                        torch.save(model_to_save.state_dict(), output_dir)
                        logger.info(
                            "Saving model checkpoint to %s", output_dir)
...
def new_main():
    ...
    if args.do_train:
        ...
        model.load_state_dict(torch.load(output_dir, map_location=args.device))
        model.to(args.device)
        train2(args, train_dataset, model, tokenizer, eval_dataset)
    # Evaluation
    results = {}
    # if args.do_test:
    #     checkpoint_prefix = f'checkpoint-best-f1/{args.model_name}'
