...
def extract_function_name(text):
    ...
    match = re.search(r'\b\w+\s+(\w+)\s*\(', text)
    return match.group(1) if match else None

def format_function_output(function_name,input_text, weights,base_line_num=1):
...
def split_input_by_lines_for_bert(input_text, tokenizer, max_length=510):
    ...
    lines_count_per_chunk = []  # 新增的列表来跟踪每个段落的行数
    current_lines_count = 0

    for line in lines:
        # Tokenize the line
        line_tokens = tokenizer.tokenize(line)
        line_tokens_length = len(line_tokens)

        # If adding the current line exceeds the max length, add the current chunk to chunks and start a new chunk
        if current_chunk_length + line_tokens_length > max_length:
            chunks.append(current_chunk)
            lines_count_per_chunk.append(current_lines_count)  # 保存当前段落的行数
            current_chunk = []
            current_chunk_length = 0
            current_lines_count = 0

        current_chunk.extend(line_tokens)
        current_chunk_length += line_tokens_length
        current_lines_count += 1  # 增加当前段落的行数

    # Add the last chunk if it's not empty
    if current_chunk:
        chunks.append(current_chunk)
        lines_count_per_chunk.append(current_lines_count)  # 保存最后一个段落的行数
    ...
    return text_chunks, lines_count_per_chunk  # 返回文本段落和对应的行数列表

def extract_lines(text, begin, end):
    # 将文本拆分为行
    lines = text.split('\n')
    
    # 检查begin和end的有效性
    if begin < 1 or end > len(lines) or begin > end:
        return "Invalid range specified"
    
    # 从列表中截取特定的行，注意Python的索引是从0开始的，所以需要做-1的调整
    selected_lines = lines[begin-1:end]
